...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, master, game, x, y, target, entity='normal'):
        super().__init__()
        self.game = game
        self.bullet_size = 7
        ...
        if self.bounce_back is False:
            for enemy in self.game.enemy_manager.enemy_list:
                if self.rect.colliderect(enemy.hitbox):
                    ...
                    self.kill()
        self.collision_enemy(self.game.player)
        self.bounce()

    def draw(self):
        radius = 5
        #self.sparkle()
        pygame.draw.circle(self.game.screen, (255, 255, 255), (self.rect.x + radius / 2, self.rect.y + radius / 2),
                           radius)
        pygame.draw.circle(self.game.screen, (58, 189, 74), (self.rect.x + radius / 2, self.rect.y + radius / 2),
                           radius - 1)
